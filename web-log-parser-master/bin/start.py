...
def parse_log_file(target_file, log_format, log_type=None):
    # 用户IP
    hosts = []
    # 访问时间
    times = []
    ...
    status_codes = {}
    if log_type == 'error':
        pattern = re.compile(config.log_pattern_error)
    else:
            ...
            if not_static_file(url):
                ...
                if not log_type:
                    log_time = time.strftime('%Y-%m-%d %H:%M:%S',
                                             time.strptime(match.group(log_format.get('time_index')),
                                                           '%d/%b/%Y:%H:%M:%S'))
                else:
    ...
    if len(times) > 2:
        cross_time = datetime.datetime.strptime(times[-1], '%Y-%m-%d %H:%M:%S') - datetime.datetime.strptime(times[0],
                                                                                                             '%Y-%m-%d %H:%M:%S')
    else:
        cross_time = None
    ...
    for item in urls_most_common:
        if item[1] >= config.urls_pv_threshold:
            ...
            continue
        if cross_time and cross_time.seconds < config.urls_pv_threshold_time and item[
            1] >= config.urls_pv_threshold_min:
            ratio = '%0.3f' % float(item[1] * 100 / float(pv))
            url_data_list.append(URLData(url=item[0], pv=item[1], ratio=ratio))
            for_url_data_uri_index.append(item[0])
            continue
    ...
    for target_file in target_files:
        try:
            ...
            if type == None:
                ...
                log_format_error = parse_log_format()
                parse_log_file(target_file, log_format_error, type)
